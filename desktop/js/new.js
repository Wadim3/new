...
function generArt(json) {
  ...
  statArt.innerHTML =
    ...
    thisjson[9] +
    "</span></div><div class='stat-item middle-stat'><img src='asset/hearth.svg' alt='Избранное' class='stat-img'><span class='stat__favorites stat__vidofa'>" +
    Number(thisjson[10]) +
...
function downloadEv(e) {
  ...
  fetch("download.php?down=" + fileUrl)
    .then(fetchHandler)
    .then(text)
    .then((res) => {
...
function reviewLike(e) {
  ...
  fetch("review_like.php?liked=" + id.getAttribute("review-id"))
    .then(fetchHandler)
    .then(text)
    .then((res) => {
      res = JSON.parse(res);
