...
if (!empty($_FILES['image']) || !empty($_POST['nick']) || !empty($_POST['about']) || !empty($_POST['pass']) && !empty($_POST['lastpass'])) {
  ...
  if (!empty($_POST['nick'])) {
    ...
    $checkReq = "SELECT `user_id` FROM `users` WHERE `nickname`='{$nick}'";
    $res = $mysqli->query($request);
    if ($res->num_rows) {
      $arrResult['err'] = "Такой никнейм уже существует";
    } else {
      $request .= "`users`.`nickname`='{$nick}',";
      $arrResult['nick'] = "1";
    }
  ...
  if (!empty($_POST['lastpass']) && !empty($_POST['pass'])) {
    ...
    if (password_verify($_POST['lastpass'], $passUser["pass"])) {
      ...
      $arrResult['err'] = "Неправильный пароль";
    }
  }
