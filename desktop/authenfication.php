...
// echo $_SESSION['hash_id'];
// SELECT
//     COUNT(DISTINCT `user_views`.`game_id`) AS 'views',
//     COUNT(DISTINCT `favorites`.`game_id`) AS 'favorites',
//     COUNT(DISTINCT `rating`.`game_id`) AS 'rating',
//     `users`.`user_id`,
//     `users`.`about`,
//     `users`.`date`,
...
//     `users`.`user_id`,
//     `users`.`picture` AS 'pict',
//     `users_session`.`user_agent` AS 'usag',
//     `users_session`.`banned` AS 'bann',
//     `users_session`.`ip`
// FROM
//     `users_session`
// LEFT JOIN `users` ON `users`.`user_id` = `users_session`.`user_id`
// LEFT JOIN `favorites` ON `favorites`.`user_id` = `users_session`.`user_id`
// LEFT JOIN `user_views` ON `user_views`.`user_id` = `users_session`.`user_id`
// LEFT JOIN `rating` ON `rating`.`user_id` = `users_session`.`user_id`
// WHERE
//     `users_session`.`agent_hash` = 'asdf'